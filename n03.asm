...
.eqv LIMIT_TIME		1000		# Gioi han thoi gian 10000 chu ki ngat

.data
	
	string: .asciiz "Go doan nay di cac tinh yeu"
	end_string: .space 1
	buffer: .space 100
...
.text
	...
	li $t8, 0	# Bien dieu kiem dem so chu ky
...
mess:	
	...
	jal SHOW_7SEG_RIGHT	# Hien thi hang don vi led trai
	jal SHOW_7SEG_LEFT	# Hien thi hang chuc led phai
...
# @return $a0 Ma hoa tung ung hien ti den led
#--------------------------------------------------------------- 	
SET_DATA_FOR_7SEG:
	beq $t0, 0, __0
	beq $t0, 1, __1
...
IntSR:
 ...
 bne $v0, 1024, exit	# Ma ngat 1024, bo qua ma ngat do Counter cua Digit Lab Sim
 			# Mat ngat khac, la Loi => Ket thuc chuong trinh
